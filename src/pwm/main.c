...
void init_pwm_static(uint32_t period, uint32_t duty_cycle) {
    // fill in
    //step 1 - config based on slice number and channel properties for 37, 38, 39
    ...
    //step 2 - set PWM slice clock divider to 150
    pwm_set_clkdiv(slice37, 150); //what do you put for pwm struct
    pwm_set_clkdiv(slice38, 150); //what do you put for pwm struct

    pwm_set_clkdiv(slice39, 150); //what do you put for pwm struct


    //step 3 - set wrapping counter of pin PWM slice to vvalue passed in period - 1
    //how to write to the register
    pwm_set_wrap(slice37, period-1);
    pwm_set_wrap(slice38, period-1);
    pwm_set_wrap(slice39, period-1);

    //NOTE: figure out how to read it
...
//NOTE: when configuring interrupts for PWM, use WRAP0
void pwm_breathing() { 
    // fill in
    //gpio_set_function(color)
    
    uint slice_col = pwm_gpio_to_slice_num(color + 37);
    uint chan_col = pwm_gpio_to_channel(color + 37);

    uint slice37 = pwm_gpio_to_slice_num(37);

    //gpio_acknowledge_irq(37, PWM_IRQ_WRAP_0);
    pwm_hw->intr = (1 << slice37);
    ...
    if(duty_cycle == 0 && dir == 1){dir = 0;}
    else if(duty_cycle == 100 &&  dir == 0){dir = 1;}
    ...
    pwm_set_chan_level(slice_col, chan_col, (duty_cycle * pwm_hw->slice[slice_col].top)/100);
...
void init_pwm_irq() {
    ...
    //how to enable interrupts for slice
    irq_set_enabled(PWM_IRQ_WRAP_0, true);

    //obtain current period for PWM?? --> use formula from datasheet
    //uint current_period = ((pwm_hw->slice[slice37].top + 1) * (PWM_CH0_CSR_PH_CORRECT_BITS + 1) * (PWM_CH0_DIV_INT_BITS + (PWM_CH0_DIV_FRAC_BITS)/16));
    
    uint16_t current_period = pwm_hw->slice[slice37].top;
    duty_cycle = 100;
    dir = 1;

    //how to set the duty cycles of 3 RGB LED PWM outputs = current period
    //from the measure_duty_cycle function from datasheet
    ...
    pwm_set_both_levels(slice37, current_period, current_period);
    pwm_set_both_levels(slice38, current_period, current_period);
    //pwm_set_both_levels(slice39, current_period, current_period);
...
void pwm_audio_handler() {
    ...
    //step 1
    //gpio_acknowledge_irq(36, PWM_IRQ_WRAP_0);
    pwm_hw->intr = (1 << slice36);
    ...
    //steps 6 + 7 (should samp be "short" or "int")
    int samp = wavetable[offset0 >> 16] + wavetable[offset1 >> 16];
    samp = samp / 2;

    //steps 8 + 9
    samp = (samp * (pwm_hw->slice[slice36].top)) / (1 << 16);

    //what is the PWM duty cycle register, and how to write to it
    pwm_set_chan_level(slice36, PWM_CHAN_A, samp);
}

void init_pwm_audio() {
    // fill in
    //NOTE: PWM IRQ WRAP0 NUMBER IS 8 (PWM_IRQ_WRAP_0)

    gpio_set_function(36, GPIO_FUNC_PWM); //added after


    //configures pin 36 as output
    uint slice36 = pwm_gpio_to_slice_num(36);
    uint chan36 = pwm_gpio_to_channel(36);

    //set corresponding PWM slice clk divider --> does pwm_set_clkdiv divide it again if I 
    //call this inside the set_channel function
    pwm_set_clkdiv(slice36, 150.f);

    //pwm_hw->slice[slice36].top = ((1000000 / RATE) - 1); 
    pwm_set_wrap(slice36, (1000000 / RATE) - 1); //NOTE: FIX THIS LINE
    ...
    //step 7
    irq_set_exclusive_handler(PWM_IRQ_WRAP_0, pwm_audio_handler); //set as handler for when
    //appropriate slice's counter register wraps around to 0

    //step 8 
    irq_set_enabled(PWM_IRQ_WRAP_0, true);

    //step 9 --> ENABLE THE PWM SLICE
