...
int note_length[2][3] = {{200000, 200000, 0}, {200000, 200000, 0}};
...
void playsound(int id){
    ...
    //make TIMER1 ALARM0 fire interrupt again in 3ms
    irq_set_exclusive_handler(TIMER1_IRQ_0, playsound_isr); //set irq handler for alarm irq (needs ALARM0??)
    //enable alarm irq
    irq_set_enabled(TIMER1_IRQ_0, true);
    hw_set_bits(&timer1_hw->inte, 1u << 0);

    // uint64_t target = timer1_hw->timerawl + 100000;
    // printf("Current time %d us\n", timer1_hw->timerawl);
    // printf("Setting next interrupt at %d us\n", (uint32_t) target);
    // timer1_hw->alarm[0] = (uint32_t) target;
